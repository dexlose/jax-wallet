...
async function fetchUtxoHistoryBlockcypherDoge(
  address: string,
  page: number,
  limit: number
): Promise<{ items: TxHistoryItem[]; hasMore: boolean }> {
  const url = `/api/doge/history?address=${encodeURIComponent(address)}&page=${page}&limit=${limit}`
  ...
  const allTx = j.txs || []
  const slice = allTx
  const hasMore = j.total ? slice.length < j.total : false
  const decimals = 8
  const symbol = "DOGE"
...
export async function fetchDogeHistoryPaged(
  address: string,
  page: number,
  limit: number
): Promise<{ items: TxHistoryItem[]; hasMore: boolean }> {
  return fetchUtxoHistoryBlockcypherDoge(address, page, limit)
}
